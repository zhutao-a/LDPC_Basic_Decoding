function [flag,final_Pe]= GA_apprx(sigma,iter,Pe,vn_deg,vn_edge_prop,cn_deg,cn_edge_prop,punc_deg,punc_prop)%给定sigma判断是否可译码
vn_deg_len=length(vn_deg);
cn_deg_len=length(cn_deg);
Ecn= zeros(cn_deg_len, 1);
punc_deg_len=length(punc_deg);

Evn0=2/sigma^2*ones(vn_deg_len, 1);%初始化变量节点从信道获取的信息Evn0(没被puncture时)
