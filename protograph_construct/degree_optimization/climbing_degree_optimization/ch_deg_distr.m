function [m_deg_per_col,m_deg_per_row]=ch_deg_distr(deg_per_col,deg_per_row,mutate_num,strategy,vn_deg_max,cn_deg_max)%通过不同策略来改变度分布
%NP表示种群大小
%strategy   1:对基础矩阵同一行不同列的0,1进行交换   2:对基础矩阵同一列不同行的0,1进行交换   3:增加一条边     4:减少一条边
%生成种群个每行每列的度分布
N=length(deg_per_col);
M=length(deg_per_row);
