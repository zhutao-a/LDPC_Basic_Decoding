...
// 根据 protograph 随机构造移位值
void cons_qc_random(const int pg[][BASE_COL],int mat[][BASE_COL]) 
{
...
// for test
void cons_qc_all0(const int pg[][BASE_COL],int mat[][BASE_COL]) 
{
	for (int i1=0;i1<BASE_ROW;i1++)
	{
		for (int i2=0;i2<BASE_COL;i2++)
...
// 统计经过每个元素的环 4 个数，保存到 r 中
void elem_count_cycle4(const int mat[][BASE_COL],int r[][BASE_COL]) 
{
	for (int i1=0;i1<BASE_ROW;i1++)
	{
		for (int i2=0;i2<BASE_COL;i2++)
...
// 根据经过的环数降序坐标列表，不包括没有环经过的元素
void argsort_elem_cycle_count(const int mat[][BASE_COL],coord_list &r) 
{
...
// 计算当 mat[i1][i2]=v 时，经过(i1,i2)的环4个数为 r[v]
void eval_c4n(const int mat[][BASE_COL],int i1,int i2,int r[]) 
{
...
// 计算当 mat[i1][i2]=v 时，经过(i1,i2)的环6个数为 r[v]
...
// 返回最佳元素
int elem_choice(const int mat[][BASE_COL],int i1,int i2) 
{
...
// 进行优化，最多调用 tmax 次 elem_choice，tmax为0表示不限
void opt_main(int mat[][BASE_COL],int tmax=0) 
{
	int tcount=0;
	while (1)
	{
...
// 掩模一个环 6 最多的元素，返回是否产生了有效掩模
bool mask_one_main(int mat[][BASE_COL]) 
{
	int vdegs[BASE_COL];
	int cdegs[BASE_ROW];
...
// IO 相关// 从文件读取矩阵
void read_matrix(int mat[][BASE_COL],const char *filename) 
{
...
// 保存矩阵到文件
void save_matrix(const int mat[][BASE_COL],const char *filename) 
{
