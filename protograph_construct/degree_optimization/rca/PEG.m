function H = PEG(N, M, Dv, ACE)%Random PEG. If you want add ACE improvement, just set 'ACE = 1'.
H = zeros(M, N);%Unsigned integer 8 is enough.
...
tree = zeros(N + M, 1);%N + M is the upper bound of the number of elements in the tree.
parent = zeros(N + M, 1);%Record the relationships in the tree.
...
%         disp(['PEG LDPC under Construction. Now ' num2str(v/N*100) '%.']);
    ...
    for k = 1 : Dv(v)%for each k, we add an '1' in H.
        if k == 1%get the 0-th level for k = 1
            ...
            H_column_one_record(H_column_one_record_index(v), v) = candidate_row(idx);%record checknode index
            H_column_one_record_index(v) = H_column_one_record_index(v) + 1;%next edge row index
            H_row_one_record(candidate_row(idx), H_row_one_record_index(candidate_row(idx))) = v;%record variablenode index
            H_row_one_record_index(candidate_row(idx)) = H_row_one_record_index(candidate_row(idx)) + 1;%next edge column index
            continue;
        ...
        while(1)
            ...
            cannot_grow_flag = 1;
            for i = marker(2 * level, 1) : marker(2 * level, 2)%CN level
