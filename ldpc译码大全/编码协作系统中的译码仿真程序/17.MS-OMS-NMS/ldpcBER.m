...
% EbN0 in dB
%EbN0_dB = [1.5 1.8 2.1 2.4 2.7 3.0];
EbN0_dB = [1:0.5:3];
%EbN0_dB = 3;
...
%semilogy(EbN0_dB,ber3,'ko-');
%semilogy(EbN0_dB,ber3,'or-');
%semilogy(EbN0_dB,ber4,'ko-');
%semilogy(EbN0_dB,ber4,'bd-');
%semilogy(EbN0_dB,ber4,'r*-');
semilogy(EbN0_dB,ber4,'k-o');
...
%h=legend('NMS(factor=0.8)','Hybird C-V Processing(factor=0.8)');
...
%h=legend('Coop-G-LDPC-NMS(a=0.8)','Coop-G-LDPC-NMS的改进');

%h=legend('Hybrid C-V Processing','NMS基于最小和次小值间距差改进','NMS(factor=0.8)','Hybrid C-V Processing(factor=0.8');
%h=legend('Hybird C-V Processing','NMS(factor=0.8)');
