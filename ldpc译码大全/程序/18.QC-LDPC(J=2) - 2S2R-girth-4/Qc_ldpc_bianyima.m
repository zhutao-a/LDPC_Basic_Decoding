...
J1=J/4;
for i=1:(L1*p-J1*p)             
 s(1,i)=round(rand(1)); %随机生成码长为(n-m)的信息比特位
...
%         tx2 =C2*r2;
        ...
        tx=[tx1 tx2 tx3 tx4];%级联
%        tx = tx./(N0/2);
        tx=tx';
%       [vhat1,vhat10] = decodeProbDomain(tx, H, iter);
%         [vhat1,vhat5,vhat10] = decode_NMS(tx, HD, iter);
        [vhat1] = decode_NMS(tx, HD, iter);
...
title('Hybrid-C-V译码算法在编码协作系统中的应用');
%title('不同译码在编码协作系统中的性能比较');
semilogy(Eb_N0, ber1, '-ko');
%semilogy(Eb_N0, ber1, '-ro',Eb_N0, ber5, '-rs',Eb_N0, ber10, '-r+');
xlabel('SNR (dB)');
ylabel('BER');
%legend('normalized-Jointly QC-LDPC iter=1','normalized-Jointly QC-LDPC iter=5','normalized-Jointly QC-LDPC iter=10');
legend('Hybrid-C-V-QC-LDPC iter=10');
grid on;
hold on;
hold off;
